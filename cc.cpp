...
////////////////////////////////////////////////////////////////
//  CC SOURCE. Aici este codul care ruleaza pe transmitatori. Tot ce avem nevoie
//  pentru a implementa un algoritm de congestion control se gaseste aici.
////////////////////////////////////////////////////////////////
int CCSrc::_global_node_count = 0;

CCSrc::CCSrc(EventList& eventlist) : EventSource(eventlist, "cc"), _flow(NULL) {
...
void CCSrc::startflow() {
  cout << "Start flow " << _flow._name << " at " << timeAsSec(eventlist().now())
       << "s" << endl;
  _flow_started = true;
  _highest_sent = 0;
  _packets_sent = 0;
...
void CCSrc::connect(Route* routeout, Route* routeback, CCSink& sink,
...
/* Variabilele cu care vom lucra:
    ...
    > Puteti include orice alte variabile in restul codului in functie de
   nevoie.
*/
/* TODO: In mare parte aici vom avea implementarea algoritmului si in functie de
 * nevoie in celelalte functii */

void CCSrc::processNack(const CCNack& nack) {
  // cout << "CC " << _name << " got NACK " <<  nack.ackno() << _highest_sent <<
  // " at " << timeAsMs(eventlist().now()) << " us" << endl;
  _nacks_received++;
  _flightsize -= _mss;
...
/* Functia de receptie, in functie de ce primeste cheama processLoss sau
 * processACK */
...
void CCSrc::send_packet() {
  ...
  p = CCPacket::newpkt(*_route, _flow, _highest_sent + 1, _mss,
                       eventlist().now());
  ...
  // Flow " << _name << " time " << timeAsUs(eventlist().now()) << endl;
  p->sendOn();
}
...
CCSink::CCSink() : Logged("CCSINK"), _total_received(0) {
  _src = 0;
...
void CCSink::receivePacket(Packet& pkt) {
  ...
  if (pkt.header_only()) {
    ...
    // cout << "Wrong seqno received at CC SINK " << seqno << " expecting " <<
    // _cumulative_ack << endl;
    return;
  }
